...
template<typename T> concept has_null_value    = requires(T x) { requires std::is_same_v<std::add_const_t<typename T::value_type>, decltype(x.nullValue)>; };
...
template<typename T> concept swappable         = any_character<T> || any_integral<T> || std::floating_point<T> || enumerated<T> || has_null_value<T>;
